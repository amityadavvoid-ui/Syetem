...
function updateParticlesLoop() {
  ...
  for (let i = activeParticles.length - 1; i >= 0; i--) {
    ...
    if (progress >= 1) {
      if (pState.element.parentNode) pState.element.parentNode.removeChild(pState.element);
    ...
    pState.element.style.opacity = currentOpacity;
    pState.element.style.transform = `translateY(${moveY}vh)`;
  }
